...
try:
    while True:
        ...
        nearest_validated_resistance = min(reslist,
                                           key=lambda x: abs(x - resistance))
        if (
            nearest_validated_resistance - resistance < 5
            and nearest_validated_resistance - resistance > 0
        ):
        ...
        m = (
            ...
            + " TRAINER TX DATA "
            + str(resistance)
            + " {}".format(" ".join(hex(x)[2:].zfill(2) for x in byte_ints))
        )
        print(m)
        log_file.write(m + "\n")
