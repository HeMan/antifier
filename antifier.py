...
parser.add_argument(
    "-l",
    "--headless",
    help="Run headless, requires -c",
    required=False,
    action="store_true",
)
parser.add_argument(
    "-c", "--power-curve", help="Choose power curve file", required=False
)
...
class PowerFactor_Window:
    def __init__(self, master):
        ...
        self.buttonPwrFact = Button(
            self.frame,
            height=1,
            width=20,
            text=u"Set power factor",
            command=self.serPwrFactorbutton,
        )
        self.buttonPwrFact.grid(column=0, row=0)

        self.PwrFactVariable = StringVar()
        self.entry = Entry(self.frame, textvariable=self.PwrFactVariable)
...
class Window(Frame):
    ...
    def init_window(self):
        ...
        subSetup.add_command(
            label="iMagic",
            command=lambda p="power_calc_factors_imagic.txt": self.settrainer(p),
        )
        subSetup.add_command(
            label="Fortius",
            command=lambda p="power_calc_factors_fortius.txt": self.settrainer(p),
        )
        subSetup.add_command(
            label="Custom Curve",
            command=lambda p="power_calc_factors_custom.txt": self.settrainer(p),
        )
            ...
            try:
                while switch:
                    ...
                    if calc_power < 0:
                        calc_power = 0
                    if debug:
                        print(speed, pedecho, heart_rate, force_index, cadence, calc_power)
                    # ##################SEND DATA TO TRAINER##################
                    ...
                    if (
                        ...
                        accumulated_time_counter = int(
                            (time.time() * 1000 - accumulated_time) / 1000 / 0.25
                        )  # time since start in 0.25 seconds
                        if (
                            accumulated_time_counter >= 256
                        ...
                        newdata = "{0}{1}{2}{3}{4}".format(
                            newdata[:24], hexspeed[2:], " ", hexspeed[:2], newdata[29:]
                        )  # set speed
                        newdata = "{0}{1}{2}".format(
                            newdata[:36], ant.calc_checksum(newdata), newdata[38:]
                        ...
                        if cadence >= 254:
                            cadence = 253
                        newdata = "{0}{1}{2}".format(
                            newdata[:18], hex(cadence)[2:].zfill(2), newdata[20:]
                        )  # instant cadence
                        ...
                        newdata = "{0}{1}{2}".format(
                            newdata[:27], hexinstant_power_lsb, newdata[29:]
                        )  # set power lsb byte
                    ...
                    if heart_rate > 0:  # i.e. heart rate belt attached
                        ...
                        hex_heart_beat_time = int(
                            (heart_beat_event_time - heart_beat_event_time_start_cycle)
                            * 1.024
                        )  # convert ms to 1/1024 of a second
